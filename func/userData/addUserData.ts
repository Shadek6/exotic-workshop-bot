...
export async function addUserData(USER_ID: string, CHAR_NAME: string, PHONE_NUMBER: string, ACCOUNT_NUMBER: string, interaction: ChatInputCommandInteraction) {
    ...
    mongoClient
        .insertOne({ user_id: USER_ID, char_name: CHAR_NAME, phone_number: PHONE_NUMBER, account_number: ACCOUNT_NUMBER })
        .then(() => {
            ...
            logMessage(0, interaction.user.username, "Register Success", `Użytkownik dodał dane postaci \`${CHAR_NAME}\` do bazy pracowników!`, interaction.user.id);
            return;
        })
        .catch(() => {
